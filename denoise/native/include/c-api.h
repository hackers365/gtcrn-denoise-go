...
// ============================================================
// For shared-engine voice activity detection
// ============================================================

/** @brief Opaque shared VAD engine handle. */
typedef struct SherpaOnnxVoiceActivityDetectorEngine
    SherpaOnnxVoiceActivityDetectorEngine;

/** @brief Opaque per-connection VAD stream handle. */
typedef struct SherpaOnnxVoiceActivityDetectorStream
    SherpaOnnxVoiceActivityDetectorStream;

/**
 * @brief Create a shared voice activity detector engine.
 *
 * The first version of this API supports Silero ONNX VAD only. The engine owns
 * a pool of model sessions. Create one engine per model and one stream per
 * audio connection. If @p pool_size <= 0, a default based on hardware
 * concurrency is used.
 *
 * @param config VAD configuration.
 ...
 */
SHERPA_ONNX_API const SherpaOnnxVoiceActivityDetectorEngine *
SherpaOnnxCreateVoiceActivityDetectorEngine(
    const SherpaOnnxVadModelConfig *config, int32_t pool_size);

/** @brief Destroy a shared VAD engine. */
SHERPA_ONNX_API void SherpaOnnxDestroyVoiceActivityDetectorEngine(
    const SherpaOnnxVoiceActivityDetectorEngine *p);

/**
 * @brief Create per-connection VAD stream state from a shared engine.
 *
 * Existing streams keep the engine internals alive until those streams are
 * also destroyed.
 */
SHERPA_ONNX_API const SherpaOnnxVoiceActivityDetectorStream *
SherpaOnnxVoiceActivityDetectorEngineCreateStream(
    const SherpaOnnxVoiceActivityDetectorEngine *engine,
    float buffer_size_in_seconds);

/** @brief Destroy a per-connection VAD stream. */
SHERPA_ONNX_API void SherpaOnnxDestroyVoiceActivityDetectorStream(
    const SherpaOnnxVoiceActivityDetectorStream *stream);

/** @brief Return the number of model sessions in the shared engine. */
SHERPA_ONNX_API int32_t SherpaOnnxVoiceActivityDetectorEngineGetPoolSize(
    const SherpaOnnxVoiceActivityDetectorEngine *engine);

/** @brief Feed mono floating-point PCM samples to a VAD stream. */
SHERPA_ONNX_API void SherpaOnnxVoiceActivityDetectorStreamAcceptWaveform(
    const SherpaOnnxVoiceActivityDetectorStream *stream, const float *samples,
    int32_t n);

/** @brief Compute the speech probability for one VAD window. */
SHERPA_ONNX_API float SherpaOnnxVoiceActivityDetectorStreamCompute(
    const SherpaOnnxVoiceActivityDetectorStream *stream, const float *samples,
    int32_t n);

/** @brief Check whether a VAD stream has no completed speech segment. */
SHERPA_ONNX_API int32_t SherpaOnnxVoiceActivityDetectorStreamEmpty(
    const SherpaOnnxVoiceActivityDetectorStream *stream);

/** @brief Check whether a VAD stream is currently inside speech. */
SHERPA_ONNX_API int32_t SherpaOnnxVoiceActivityDetectorStreamDetected(
    const SherpaOnnxVoiceActivityDetectorStream *stream);

/** @brief Remove the front speech segment from a VAD stream. */
SHERPA_ONNX_API void SherpaOnnxVoiceActivityDetectorStreamPop(
    const SherpaOnnxVoiceActivityDetectorStream *stream);

/** @brief Remove all queued speech segments from a VAD stream. */
SHERPA_ONNX_API void SherpaOnnxVoiceActivityDetectorStreamClear(
    const SherpaOnnxVoiceActivityDetectorStream *stream);

/** @brief Get a copy of the front speech segment from a VAD stream. */
SHERPA_ONNX_API const SherpaOnnxSpeechSegment *
SherpaOnnxVoiceActivityDetectorStreamFront(
    const SherpaOnnxVoiceActivityDetectorStream *stream);

/** @brief Reset a VAD stream so it can process a new audio connection. */
SHERPA_ONNX_API void SherpaOnnxVoiceActivityDetectorStreamReset(
    const SherpaOnnxVoiceActivityDetectorStream *stream);

/** @brief Flush buffered tail samples and force final segmentation. */
SHERPA_ONNX_API void SherpaOnnxVoiceActivityDetectorStreamFlush(
    const SherpaOnnxVoiceActivityDetectorStream *stream);
